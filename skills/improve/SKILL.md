...
3. **Write tasks, not findings.** Append to `## Tasks` (never edit or reorder existing tasks). Each task must survive execution by a fresh model with zero memory of this audit:
   ...
   - The Verify command must be runnable from the repo root and must fail before the fix / pass after. If you can't state one, the task isn't ready, so drop it or mark it `NEEDS-HUMAN`.
