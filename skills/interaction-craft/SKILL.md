...
> **Canonical source:** Emil Kowalski's paid course "Animations on the Web" at https://animations.dev. This skill is a distillation; when a question isn't answered here, the course is the reference of record. Do not rebuild or re-derive the doctrine from scratch.

## The frequency rule (decide first: should this animate?)

- **Rare** (onboarding, empty states, completions, marketing/portfolio): delight allowed, including morphing, springs, and staggered reveals.
...
- **Celebration** (rare, meaningful events only, such as a lesson complete or a goal shipped): SVG stroke draw-in or scale spring, ≤400ms.
- **Count-up numbers** on rarely-viewed stat tiles.
- **Nested menus, the safe triangle:** when the cursor moves diagonally toward a submenu it briefly leaves the parent item's bounds; native UIs (macOS, Windows, Amazon) keep the submenu open by tracking the triangle between cursor and submenu corners. Don't hand-roll this: Radix UI dropdown/menubar primitives implement it. Before building any nested menu, check whether the app already uses Radix and build on that.
