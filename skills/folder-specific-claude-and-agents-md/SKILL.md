...
Ask the user which folder, and use its absolute path (e.g. somewhere under `~/`).
...
Before writing the file, give the user a bullet list grouped by section, and let them react first. Candidate sections (skip any that don't apply):
